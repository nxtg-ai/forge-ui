...
### Command Failures
- `/nxtg-init` failed? Check if .claude directory exists
- `/nxtg-status` error? Verify git repository is initialized
- `/nxtg-feature` blocked? Ensure clean working directory
- `/nxtg-test` failing? Check test framework installation
- `/nxtg-deploy` stopped? Verify deployment credentials

### Agent Issues
- Agent not responding? Use `/nxtg-status` to check health
- Orchestration stuck? Run `/nxtg-report` for activity log
- Quality gates failing? Review with `/nxtg-status-enhanced`
