...
export interface MCPServer {
  ...
  name: string;
  description: string;
  category: 'database' | 'api' | 'productivity' | 'development' | 'automation' | 'design';
  useCase: string;
  benefits: string[];
  command: string;
  args?: string[];
 ...
 */
const MCP_REGISTRY: Record<string, Omit<MCPServer, 'priority' | 'relevanceScore' | 'reasoning'>> = {
  'github': {
    id: 'github',
...
export class MCPSuggestionEngine {
  ...
  async suggestMCPs(vision: VisionContext): Promise<MCPSuggestion> {
    ...
    const recommended = scoredServers.filter(s => s.priority === 'recommended');
    const optional = scoredServers.filter(s => s.priority === 'optional');

    // Step 3: Calculate setup time
    const totalEstimatedSetupTime = this.calculateSetupTime(essential, recommended);
...
${Object.values(MCP_REGISTRY).map(mcp => `- ${mcp.name}: ${mcp.description}`).join('\n')}
...
`;
    ...
    try {
      ...
      const { stdout } = await execAsync(`echo "${escapedPrompt}" | claude --dangerously-skip-permissions`);
      ...
      throw new Error(`Failed to analyze vision: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private calculateSetupTime(essential: MCPServer[], recommended: MCPServer[]): string {
