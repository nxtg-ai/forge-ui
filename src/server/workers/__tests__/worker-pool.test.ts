...
vi.mock('crypto', () => ({
  randomUUID: vi.fn(() => 'test-uuid-' + Math.random().toString(36).slice(2, 7)),
}));
...
describe('TaskQueue', () => {
  ...
  describe('priority ordering', () => {
    it('should order by priority: high > medium > low > background', () => {
      const priorities: TaskPriority[] = ['background', 'low', 'medium', 'high'];
  ...
  describe('edge cases', () => {
    ...
    it('should handle concurrent priority changes', () => {
      ...
      for (let i = 0; i < 20; i++) {
        const priority = ['high', 'medium', 'low', 'background'][i % 4] as TaskPriority;
