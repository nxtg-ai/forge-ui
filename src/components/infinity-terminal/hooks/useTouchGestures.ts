...
export function useTouchGestures(
  ...
  const handleTouchEnd = useCallback((e: TouchEvent) => {
    ...
    if (touchStartRef.current && e.changedTouches.length === 1) {
      ...
      const now = Date.now();
      if (now - lastTapRef.current < doubleTapDelay && Math.abs(dx) < 10 && Math.abs(dy) < 10) {
        onDoubleTap?.();
  ...
  }, [doubleTapDelay, swipeThreshold, onDoubleTap, onSwipeDown, onSwipeLeft, onSwipeRight, onSwipeUp]);
