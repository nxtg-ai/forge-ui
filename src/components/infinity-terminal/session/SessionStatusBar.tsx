...
import { Cloud, CloudOff, RefreshCw, ChevronDown, Wifi, WifiOff } from 'lucide-react';
...
export const SessionStatusBar: React.FC<SessionStatusBarProps> = ({
  ...
  const { sessionName, connected, connecting, reconnectAttempts, lastActivity } = state;
    ...
    >
      <div className="flex items-center gap-2 min-w-0">
        ...
        >
          ...
          <span className="font-mono truncate">{sessionName || 'No session'}</span>
          {onShowHistory && (
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={`text-xs ${connected ? 'text-green-400' : connecting ? 'text-yellow-400' : 'text-red-400'}`}>
          {connected ? 'Connected' : connecting ? `Reconnecting${reconnectAttempts > 0 ? ` (${reconnectAttempts})` : ''}...` : 'Disconnected'}
        </span>
        ...
        <button
          ...
          title="Reconnect"
        >
          <RefreshCw className={`w-3.5 h-3.5 text-gray-400 ${connecting ? 'animate-spin' : ''}`} />
        </button>
      </div>
    </div>
  );
