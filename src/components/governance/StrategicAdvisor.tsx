    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className={`text-2xs font-mono ${config.text}`}>
      ...
      {/* Recommended Action */}
      <div
        className={`${config.bg} border ${config.border} rounded-md p-3 mb-3`}
      >
        <div className="flex items-start gap-2">
          <ArrowRight
            className={`w-4 h-4 ${config.text} mt-0.5 flex-shrink-0`}
          />
          <div>
      ...
      {/* Reasoning */}
      <div className="mb-3">
        <p className="text-2xs font-medium text-gray-400 mb-1.5">
...
function analyzeStateAndRecommend(state: GovernanceState): Recommendation {
  ...
  const completedWorkstreams = workstreams.filter(
  ...
  const averageProgress =
    workstreams.reduce((sum, w) => sum + w.progress, 0) / totalWorkstreams;

  const criticalEvents = sentinelLog.filter(
    (log) => log.type === "CRITICAL" || log.type === "ERROR",
  ).length;
  const recentEvents = sentinelLog.filter(
  ...
  if (activeWorkstreams > 0 && averageProgress < 100) {
    const highRiskWorkstreams = workstreams.filter(
      (w) => w.risk === "high" || w.risk === "critical",
    ).length;
