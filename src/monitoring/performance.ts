...
const PERFORMANCE_THRESHOLDS: Record<MetricType, { warning: number; critical: number }> = {
...
export class PerformanceMonitor extends EventEmitter {
  ...
  start(reportIntervalMs: number = 60000): void {
    logger.info('Starting performance monitoring', { interval: reportIntervalMs });
   ...
   */
  startOperation(id: string, type: MetricType, metadata?: Record<string, any>): void {
   ...
   */
  endOperation(id: string, type: MetricType, success: boolean = true, error?: string): void {
   ...
   */
  trackCommandExecution(command: string, duration: number, success: boolean = true): void {
    this.recordMetric({
   ...
   */
  private calculateStats(metrics: PerformanceMetric[]): PerformanceStats | null {
  ...
  generateReport(): PerformanceReport {
    ...
    for (const [key, metrics] of this.metrics) {
      ...
      if (typeStats) {
        ...
        totalErrors += Math.round(typeStats.count * (1 - typeStats.successRate / 100));
