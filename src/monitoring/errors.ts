...
export class ErrorTracker extends EventEmitter {
  ...
  constructor(projectPath?: string) {
    ...
    this.recoveryStrategies = new Map(
      Object.entries(DEFAULT_RECOVERY_STRATEGIES) as [
        ErrorCategory,
        RecoveryStrategy,
      ][],
    );
  ...
  generateReport(): ErrorReport {
    ...
    for (const stats of categories.values()) {
      const categoryErrors = Array.from(this.errors.values()).filter(
        (e) => e.category === stats.category,
      );
      const totalAttempts = categoryErrors.reduce(
        (sum, e) => sum + e.recoveryAttempts,
        0,
      );
      stats.averageRecoveryAttempts =
        categoryErrors.length > 0 ? totalAttempts / categoryErrors.length : 0;
    }
    ...
    const criticalErrors = Array.from(this.errors.values()).filter(
      (e) => e.severity === ErrorSeverity.CRITICAL && !e.recovered,
    );

    const errorRate = period > 0 ? (totalErrors / period) * 60 : 0; // errors per minute
    const recoveryRate =
      totalErrors > 0 ? (resolvedErrors / totalErrors) * 100 : 100;
