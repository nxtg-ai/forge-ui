...
import { HealthMonitor, SystemHealth, HealthStatus, HealthCheckType } from './health';
import { PerformanceMonitor, PerformanceReport, MetricType, PerformanceAlert } from './performance';
 ...
 */
const DashboardProgressBar: React.FC<ProgressBarProps> = ({
  value,
  max = 100,
  label,
  showPercentage = true,
          ...
          {sparkline.map((v, i) => {
            ...
            return (
              <div
                ...
                style={{ '--sparkline-height': `${heightPercent}%`, height: 'var(--sparkline-height)' } as React.CSSProperties}
              />
            );
          })}
        </div>
 ...
 */
const AlertItem: React.FC<AlertItemProps> = ({ severity, message, timestamp }) => {
...
export const MetricsDashboard: React.FC<MetricsDashboardProps> = ({
  ...
  useEffect(() => {
    ...
    performanceMonitor.current.on('performanceReport', (report: PerformanceReport) => {
      setPerformance(report);
    });

    performanceMonitor.current.on('performanceAlert', (alert: PerformanceAlert) => {
      setAlerts(prev => [...prev.slice(-9), { ...alert, id: Date.now().toString() }]);
    });
  ...
  // Get status color
  const getStatusColor = (score: number): 'healthy' | 'warning' | 'critical' => {
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <span className="ml-1 font-medium">{formatUptime(health.uptime)}</span>
              </div>
              <div>
                <span className="text-gray-500">Errors:</span>
                <span className="ml-1 font-medium">{errors?.totalErrors || 0}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-lg shadow mb-6">
          <div className="border-b border-gray-200">
            <nav className="flex -mb-px">
              {(['overview', 'performance', 'errors', 'alerts'] as const).map(tab => (
                <button
                  key={tab}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <MetricCard
                ...
                trend={health.overallScore > 85 ? 'up' : health.overallScore < 70 ? 'down' : 'stable'}
              />
              <MetricCard
                title="Uptime"
              ...
              <MetricCard
                ...
                status={errors && errors.errorRate > 10 ? 'critical' : errors && errors.errorRate > 5 ? 'warning' : 'healthy'}
              />
              <MetricCard
                title="Active Agents"
                value={health.checks.find(c => c.type === HealthCheckType.AGENT_EXECUTION)?.details?.available || 0}
                ...
                {health.checks.map(check => (
                  <div key={check.type} className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-medium text-sm">{check.type.replace(/_/g, ' ').toUpperCase()}</p>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <MetricCard
                ...
                status={performance.averageLatency > 1000 ? 'warning' : 'healthy'}
              />
              <MetricCard
                title="Error Rate"
                value={`${performance.errorRate.toFixed(1)}%`}
                subtitle="Failed operations"
                status={performance.errorRate > 10 ? 'critical' : performance.errorRate > 5 ? 'warning' : 'healthy'}
              />
            </div>

            {/* Metrics by Type */}
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-lg font-semibold mb-4">Performance by Operation Type</h2>
              <div className="space-y-4">
                {Array.from(performance.metrics.entries()).map(([type, stats]) => (
                  <div key={type} className="border-b border-gray-200 pb-3">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <p className="font-medium">{type.replace(/_/g, ' ').toUpperCase()}</p>
                        <p className="text-xs text-gray-500">{stats.count} operations</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-semibold">{stats.successRate.toFixed(1)}% success</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-4 gap-2 text-xs">
                      <div>
                        <span className="text-gray-500">Avg:</span>
                        <span className="ml-1 font-medium">{stats.averageDuration.toFixed(0)}ms</span>
                      </div>
                      <div>
                        <span className="text-gray-500">P50:</span>
                        <span className="ml-1 font-medium">{stats.p50.toFixed(0)}ms</span>
                      </div>
                      <div>
                        <span className="text-gray-500">P90:</span>
                        <span className="ml-1 font-medium">{stats.p90.toFixed(0)}ms</span>
                      </div>
                      <div>
                        <span className="text-gray-500">P99:</span>
                        <span className="ml-1 font-medium">{stats.p99.toFixed(0)}ms</span>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <MetricCard
                ...
                status={errors.errorRate > 10 ? 'critical' : errors.errorRate > 5 ? 'warning' : 'healthy'}
              />
              <MetricCard
                title="Recovery Rate"
                value={`${errors.recoveryRate.toFixed(1)}%`}
                subtitle="Recovered errors"
                status={errors.recoveryRate < 50 ? 'critical' : errors.recoveryRate < 75 ? 'warning' : 'healthy'}
              />
              <MetricCard
                title="Critical Errors"
                value={errors.criticalErrors.length}
                subtitle="Unresolved"
                status={errors.criticalErrors.length > 0 ? 'critical' : 'healthy'}
              />
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Array.from(errors.categories.entries()).map(([category, stats]) => (
                  <div key={category} className="text-center">
                    ...
                    <div className="mt-2 flex justify-center space-x-2 text-xs">
                      ...
                      <span className="text-red-600">{stats.unresolved} active</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {errors.topErrors.length > 0 && (
              <div className="bg-white rounded-lg shadow p-6">
                <h2 className="text-lg font-semibold mb-4">Most Frequent Errors</h2>
                <div className="space-y-3">
                  {errors.topErrors.slice(0, 5).map(error => (
                    <div key={error.id} className="flex justify-between items-start border-b border-gray-200 pb-2">
                      <div className="flex-1">
