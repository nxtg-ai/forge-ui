...
describe('CheckpointManager', () => {
  ...
  beforeEach(async () => {
    testProjectPath = path.join(process.cwd(), '.test-checkpoint-' + Date.now());
  ...
  describe('initialize', () => {
    ...
    it('should return Ok result', async () => {
      const newManager = new CheckpointManager(path.join(testProjectPath, 'new'));
  ...
  describe('saveCheckpoint and restoreFromCheckpoint', () => {
    ...
    it('should preserve all checkpoint data', async () => {
      ...
      expect(restored.executionState.artifacts).toEqual(['file1.ts', 'file2.ts']);
    });
  ...
  describe('clearCheckpoint', () => {
    ...
    it('should not throw for non-existent checkpoint', async () => {
      await expect(manager.clearCheckpoint('non-existent')).resolves.not.toThrow();
    });
  });
  ...
  describe('data validation', () => {
    ...
    it('should handle checkpoint metadata', async () => {
      ...
      expect(result.unwrap().metadata).toEqual({ session: 'abc', branch: 'feature' });
    });
  });
});
