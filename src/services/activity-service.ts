...
export class ActivityService extends BaseService {
  ...
  getActivityHistory(filter?: ActivityFilter): ActivityEvent[] {
    ...
    if (filter) {
      ...
      if (filter.visibility) {
        events = events.filter(e => e.visibility.includes(filter.visibility!));
      }
  ...
  getStatistics(filter?: ActivityFilter): ActivityStatistics {
    ...
    if (events.length > 1) {
      const timeRange = events[events.length - 1].timestamp.getTime() - events[0].timestamp.getTime();
      const minutes = timeRange / 60000;
      averageEventsPerMinute = minutes > 0 ? events.length / minutes : 0;
    }

    // Find most active agent
    const mostActiveAgent = Object.entries(eventsPerAgent)
      .sort(([, a], [, b]) => b - a)[0]?.[0] || '';
   ...
   */
  getAgentPerformance(agentId: string): Result<AgentPerformance, IntegrationError> {
   ...
   * Update agent status
   */
  async updateAgentStatus(agent: Agent): Promise<Result<void, IntegrationError>> {
    try {
  ...
  async exportActivityLog(
    ...
    try {
      if (format === 'json') {
        ...
        // CSV export
        const headers = ['ID', 'Type', 'Agent ID', 'Agent Name', 'Timestamp', 'Importance', 'Category'];
  ...
  private updateAgentPerformance(event: ActivityEvent): void {
    ...
    performance.successRate = totalTasks > 0
      ? performance.tasksCompleted / totalTasks
      : 0;
   ...
   */
  private validateActivityEvent(event: ActivityEvent): Result<void, IntegrationError> {
    const ActivityEventSchema = z.object({
      ...
      visibility: z.array(z.enum(['ceo', 'vp', 'engineer', 'builder', 'founder'])),
