...
describe('ApprovalQueueService', () => {
  ...
  describe('requestApproval', () => {
    ...
    it('should increment total requests stat', async () => {
      ...
      await service.requestApproval(context, DecisionImpact.LOW, DecisionRisk.LOW);
      const statsAfter = service.getStats();

      expect(statsAfter.approved + statsAfter.rejected + statsAfter.pending).toBe(
  ...
  describe('approve', () => {
    ...
    it('should emit approval-decided event', async () => {
      ...
      const result = await service.approve(request.id, ApproverRole.CEO, 'Approved');
  ...
  describe('reject', () => {
    ...
    it('should emit approval-decided event', async () => {
      ...
      await service.reject(request.id, ApproverRole.CEO, 'Not aligned with vision');
  ...
  describe('getPending', () => {
    it('should return all pending requests', async () => {
      ...
      await service.requestApproval(context, DecisionImpact.LOW, DecisionRisk.LOW);
      await service.requestApproval(context, DecisionImpact.MEDIUM, DecisionRisk.LOW);
    ...
    it('should filter by approver role', async () => {
      ...
      await service.requestApproval(context, DecisionImpact.LOW, DecisionRisk.LOW);
      await service.requestApproval(context, DecisionImpact.HIGH, DecisionRisk.MEDIUM, {
        requiredApprover: ApproverRole.ARCHITECT,
      });
      await service.requestApproval(context, DecisionImpact.MEDIUM, DecisionRisk.MEDIUM, {
      ...
      expect(architectPending.some(r => r.requiredApprover === ApproverRole.ARCHITECT)).toBe(true);

      // Should NOT include DESIGNER-required request
      expect(architectPending.some(r => r.requiredApprover === ApproverRole.DESIGNER)).toBe(false);
    });
    ...
    it('should not include approved/rejected/cancelled requests', async () => {
      ...
      const req1 = await service.requestApproval(context, DecisionImpact.LOW, DecisionRisk.LOW);
      const req2 = await service.requestApproval(context, DecisionImpact.LOW, DecisionRisk.LOW);
      const req3 = await service.requestApproval(context, DecisionImpact.LOW, DecisionRisk.LOW);
  ...
  describe('cancel', () => {
    ...
    it('should throw error for non-pending request', async () => {
      ...
      await expect(service.cancel(request.id)).rejects.toThrow('is not pending');
    });
  });
