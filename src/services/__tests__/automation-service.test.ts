...
import { AutomationService, AutomationRule, AutomationContext } from '../automation-service';
...
describe('AutomationService', () => {
  ...
  describe('rollback functionality', () => {
    ...
    it('should create rollback snapshot', async () => {
      ...
      if (result.isOk()) {
        ...
        expect(result.value.rollbackSnapshot?.actionId).toBe(result.value.action.id);
      }
    });
    ...
    it('should rollback action', async () => {
      ...
      if (execResult.isOk()) {
        const rollbackResult = await service.rollback(execResult.value.action.id);
        expect(rollbackResult.isOk()).toBe(true);

        const history = service.getActionHistory();
        const rolledBack = history.find(a => a.id === execResult.value.action.id);
  ...
  describe('automation rules', () => {
    ...
    it('should apply confidence modifier from rule', async () => {
      ...
      const suggestions = await service.analyzeSituation({ context: 'test case' });
      expect(suggestions.isOk()).toBe(true);
      if (suggestions.isOk()) {
        const modified = suggestions.value.find(a => a.description === 'Fix with modifier');
  ...
  describe('statistics tracking', () => {
    ...
    it('should calculate success rate', async () => {
      const successAction: Omit<AutomatedAction, 'id' | 'status' | 'timestamp'> = {
        type: 'fix',
        title: 'Success',
